...
void computeNormal(int i, int j) {
    float v1[3], v2[3], nVec[3]; // vectores normais

    // fill the normal vector with the normal for vertex at grid location (i,j)
    // Vector v1 = p2(i, h(i,j+1), j+1) - p1(i, h(i,j−1), j−1) -----
    v1[0] = 0; // (i-i) = 0
    v1[1] = h(i,j+1) - h(i,j-1);
    v1[2] = 2; // (j+1) - (j−1) = 2

    // Vector v2 = p4(i+1, h(i+1,j), j) - p3(i-1, h(i-1,j), j) -----
    v2[0] = 2; // (i+1) - (i-1) = 2
    v2[1] = h(i+1,j) - h(i-1,j);
    v2[2] = 0; // (j-j) = 0

    // Calculo do Vector Normal : n = (v1*v2) / |(v1*v2)|
    cross(v1,v2,nVec);
    normalize(nVec);

    // Gravacao das normais no buffer
    normal.push_back(nVec[0]);
    normal.push_back(nVec[1]);
    normal.push_back(nVec[2]);
}
...
void prepareTerrain() {
    ...
    for (int i = 1; i < imageWidth - 2; i++) {
        for (int j = 1; j < imageWidth - 1; j++) {
            ...
            // pretende-se usar o modelo Tiled, sendo a textura repetida por cada célula
...
void loadTexture() {
    ...
    ////Memory required for mipmap levels (+- 33%); (Better quality and Potentially faster due to cache use)
