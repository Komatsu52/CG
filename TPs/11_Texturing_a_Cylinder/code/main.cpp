...
void prepareCylinder(float altura, float raio, int lados) {
    ...
    for (int i = 0; i < lados; ++i) {
        ...
        v[vertex*3 + 2] = 0.0f;
        t[vertex*2 + 0] = 0.4375f;       // (t) - coordenadas de textura
        ...
        t[vertex*2 + 1] = 1;  // 1 - altura da textura lateral superior (tmax)
        ...
        v[vertex*3 + 2] = raio * cos( i * delta);
        t[vertex*2 + 0] = (1.0f/lados) * i;
        t[vertex*2 + 1] = 0.375f;  // 0.375 - altura da textura lateral inferior
...
void drawFloor(float xmin, float xmax, float zmin, float zmax) {
    ...
    glBegin(GL_QUADS);
        glNormal3f(0, 1, 0);
        //Por cada vertice, definimos a sua coordenada de textura
        glTexCoord2f(1, 0);
...
void drawCylinder() {
    ...
    // Definir a semântica. Buffer de Texturas
    glBindBuffer(GL_ARRAY_BUFFER, texCoord);
    glTexCoordPointer(2, GL_FLOAT, 0, 0);

    // Texturas Ligadas durante o desenho do cilindro
    glBindTexture(GL_TEXTURE_2D,texIDCylinder);
    glDrawArrays(GL_TRIANGLES, 0, vertexCount);   // desenho dos vertices do triangulo
    glBindTexture(GL_TEXTURE_2D,0);
}
...
int loadTexture(std::string s) {
    ...
    glGenTextures(1,&texID);                                          //texture buffer/slot
    glBindTexture(GL_TEXTURE_2D,texID);                                  //bind the slot
